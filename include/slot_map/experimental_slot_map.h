...
template <class T>
class const_iterator : public T {};

//struct iterator {
//  // These 5 aliases (typedefs) are required.
//  using value_type = int;         // almost always T
//  using pointer = value_type*;    // almost always T*
//  using reference = value_type&;  // almost always T&
//  using iterator_category =
//      std::random_access_iterator_tag;     // your iterator type
...
//  }
//
//  iterator& operator-=(const difference_type off) { return *this += -off; }
//  iterator operator-(const difference_type off) const {
//    iterator ret = *this;
//    return ret -= off;
//  }
...
//  }
//
//  reference operator[](const difference_type off) const { return _ptr[off]; }
//
...
//  bool operator>=(const iterator& rhs) const { return !(*this < rhs); }
//
//  friend iterator operator+(const difference_type, iterator);
...
//
//  const_iterator(mutable_pointer ptr) : _ptr(ptr) {}
...
//  bool operator>=(const const_iterator& rhs) const { return !(*this < rhs); }
//
//  friend const_iterator operator+(const difference_type, const_iterator);
//  friend void swap(const_iterator&, const_iterator&);
//
// private:
//  mutable_pointer _ptr{nullptr};
//};
//
//// iterator
//iterator operator+(const iterator::difference_type off, iterator it) {
//  return it += off;
//}
...
//// const_iterator
//const_iterator operator+(
//    const const_iterator::difference_type off,
//    const_iterator it) {
//  return it += off;
//}
//void swap(const_iterator& lhs,
//          const_iterator& rhs) {
//  using std::swap;
...
public:
    ...
    //};
    using iterator = typename slot_map_detail::iterator<typename container_type::iterator>;
    using const_iterator = typename slot_map_detail::const_iterator<typename container_type::const_iterator>;
...
template <class T>
constexpr void iter_swaptest(
    slot_map_detail::iterator<T> a) {

}

//template <class T>
//constexpr void iter_swaptest2(
...
constexpr void iter_swap(
    typename slot_map<T, Key, Container>::iterator a,
    typename slot_map<T, Key, Container>::iterator b) {
  printf("slot_map iter_swap\n");
  using std::swap;
  swap(*a, *b);
}
}  // namespace std

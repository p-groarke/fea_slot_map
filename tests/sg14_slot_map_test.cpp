...
auto get(const K& k) {
	return get(k, std::integral_constant<int, I>{});
}
template <int I, class K>
auto& get(K& k) {
	return get(k, std::integral_constant<int, I>{});
...
const uint16_t& get(const key_16_8_t& k, std::integral_constant<int, 0>) {
	return k.index;
}
const uint8_t& get(const key_16_8_t& k, std::integral_constant<int, 1>) {
	return k.generation;
}
uint16_t& get(key_16_8_t& k, std::integral_constant<int, 0>) {
	return k.index;
}
uint8_t& get(key_16_8_t& k, std::integral_constant<int, 1>) {
	return k.generation;
}
...
struct Vector {
	...
	Vector() = default;
	template <class T_ = T,
			class = std::enable_if_t<std::is_copy_constructible<T_>::value>>
...
struct Monad {
	using T = T_;
	static T value_of(const T& i) {
		return i;
	}
	template <class U>
	static T from_value(const U& v) {
...
struct Monad<std::unique_ptr<T_>> {
	...
	static T_ value_of(const T& ptr) {
		return *ptr;
	}
	template <class U>
	static T from_value(const U& v) {
		return std::make_unique<T_>(v);
	}
};
...
template <class T,
		class U = std::conditional_t<std::is_copy_constructible<T>::value,
				const T&, T&&>>
static U move_if_necessary(T& value) {
...
static void BasicTests(T t1, T t2) {
	...
	assert(sm.find_unchecked(k2) == sm.begin()); // find a non-expired key
	assert(sm[k2] == *sm.begin()); // find a non-expired key
	assert(sm.at(k2) == *sm.begin()); // find a non-expired key
	assert(sm.at_unchecked(k2) == *sm.begin()); // find a non-expired key
...
static void EraseRangeTest() {
	...
	auto test = [&](int N, int first, int last) {
		...
		for (auto it = std::next(sm.begin(), first);
				it != std::next(sm.begin(), last); ++it) {
...
static void PartitionTest() {
	...
	auto pivot = sm.partition(
			[](const auto& elt) { return Monad<T>::value_of(elt) >= 5; });
...
static void ReserveTest() {
	...
	static_assert(
			std::is_same<decltype(original_cap), typename SM::size_type>::value,
			"");
...
static void TypedefTests() {
	if (true) {
		...
		static_assert(
				std::is_same<typename SM::key_index_type, unsigned>::value, "");
		static_assert(
				std::is_same<typename SM::key_generation_type, unsigned>::value,
				"");
		static_assert(std::is_same<typename SM::container_type,
							  std::vector<int>>::value,
				"");
		static_assert(std::is_same<typename SM::reference, int&>::value, "");
		static_assert(
				std::is_same<typename SM::const_reference, const int&>::value,
				"");
	...
	if (true) {
		...
		static_assert(std::is_same<typename SM::container_type,
				...
				"");
		static_assert(std::is_same<typename SM::reference,
							  std::vector<bool>::reference>::value,
				"");
		static_assert(std::is_same<typename SM::const_reference,
							  std::vector<bool>::const_reference>::value,
				"");
		static_assert(std::is_same<typename SM::pointer,
							  std::vector<bool>::pointer>::value,
				"");
		...
		static_assert(std::is_same<typename SM::const_reverse_iterator,
				...
				"");
		static_assert(std::is_same<typename SM::size_type,
							  std::vector<bool>::size_type>::value,
				"");
	...
	if (true) {
		...
		static_assert(
				...
				"");
		static_assert(
				std::is_same<typename SM::mapped_type, double>::value, "");
		static_assert(
				std::is_same<typename SM::key_index_type, uint16_t>::value, "");
		static_assert(
				std::is_same<typename SM::key_generation_type, uint8_t>::value,
				"");
		static_assert(std::is_same<typename SM::container_type,
	...
	if (true) {
		...
		static_assert(std::is_same<typename SM::mapped_type, int>::value, "");
		static_assert(
				std::is_same<typename SM::key_index_type, char>::value, "");
		static_assert(
				std::is_same<typename SM::key_generation_type, int>::value, "");
		static_assert(std::is_same<typename SM::container_type,
							  TestContainer::Vector<int>>::value,
				"");
		static_assert(std::is_same<typename SM::reference, int&>::value, "");
		static_assert(
				std::is_same<typename SM::const_reference, const int&>::value,
		...
		static_assert(
				...
				"");
		static_assert(
				std::is_same<typename SM::size_type, unsigned>::value, "");
		static_assert(std::is_same<typename SM::value_type, int>::value, "");
	}
#if __cplusplus >= 201703L
	if (true) {
...
void slot_map_test() {
	...
	FullContainerStressTest<slot_map_7>(
			[]() { return std::make_unique<int>(1); });
	InsertEraseStressTest<slot_map_7>(
			[i = 7]() mutable { return std::make_unique<int>(++i); });
...
concept bool SlotMapContainer = requires(Ctr<T> c, const Ctr<T> cc, T t) {
